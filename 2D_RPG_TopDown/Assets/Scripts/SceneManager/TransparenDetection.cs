...
{
    [Range(0, 1)]
    [SerializeField] private float transparencyAmount = 0.8f;//Độ mờ
    [SerializeField] private float fadeTime = 0.4f;//Thời gian làm mờ
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.gameObject.GetComponent<PlayerController>())
        {
            if (spriteRenderer)
            {
                StartCoroutine(FadeRoutine(spriteRenderer, fadeTime, spriteRenderer.color.a, 1f));//Không bị mờ khi player ra khỏi
            }
            else if(tilemap)
            {
                StartCoroutine(FadeRoutine(tilemap, fadeTime, tilemap.color.a, 1f));//Không bị mờ khi player ra khỏi
            }  
        }
    }
    ...
    private IEnumerator FadeRoutine(Tilemap tilemap, float fadeTime, float startValue, float targetTransparency)
    {
        ...
        while (elapsedTime < fadeTime)
        {
            elapsedTime += Time.deltaTime;
            float newAlpha = Mathf.Lerp(startValue, targetTransparency, elapsedTime / fadeTime);
            tilemap.color = new Color(tilemap.color.r, tilemap.color.g, tilemap.color.b, newAlpha);//Đổi màu
            yield return null;//Không delay
        }
    }
}
